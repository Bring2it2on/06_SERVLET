...
public class PrintLoginSuccessServlet extends HttpServlet {
    ...
    * 깊은 복사를 이용해 값을 그대로 복사했기 때문에 내부에 존재하는 헤더나 인스턴스는 그대로 유지하고 있음.
    * */
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        /*
        * 기본적으로 변수의 기본 스코프 메소드 (= 해당 페이지) 이기 때문에 다른 페이지(=서블릿)으로
        * 데이터를 공유할 수 없다.
        * 하지만 forward 방식은 request와 response를 포함하여 위임하므로 request 정보를 저장하여,
        ...
        * 하게 되면 동일한 요청을 반복하여 데이터 베이스에 insert 하는 등의 행위가 중복될 가능성이 있다.
